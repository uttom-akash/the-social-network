...
import Feeling from "../card/CustomFeeling";
import {
  WindowScroller,
  List,
  CellMeasurer,
...
class LazyListing extends React.Component {
  ...
  onStar = (rating, index) => {
    ...
    feedApi
      .starPost({ postId: modifiedPost._id, rating: myRating })
      .then((resp) => {
  ...
  onUnstar = (index) => {
    ...
    feedApi.unstarPost({ postId: post._id, userId }).then((resp) => {
      post.totalStars =
        post.totalStars - post.ratings[post.myIndex].numberOfStars;
      post.myIndex = -10;
  ...
  rowRender = ({ index, key, style, parent }) => {
    ...
    return (
      <CellMeasurer key={key} cache={this.cache} parent={parent} index={index}>
        {({ measure }) => (
          <div style={style}>
            <IStoryCard story={item} />
